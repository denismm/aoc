...
State = NamedTuple("State", [('location', Position), ('time', int)])
start_state = State(start_position, 0)
...
while (queue):
    ...
    for direction in directions:
        ...
        if new_position in open_spaces and new_position not in next_winds:
            seen_state = State(new_position, next_tick)
            if seen_state not in seen:
                seen.add(seen_state)
                queue.append(State(new_position, state.time + 1))
print("no way out")
