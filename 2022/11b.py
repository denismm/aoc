...
monkey_tests = set([monkey.test for monkey in monkeys])
monkey_modulus = functools.reduce(lambda a, b: a * b, monkey_tests)
print(f"Monkey modulus: {monkey_modulus}")
...
for _ in range(10000):
    for i in range(len(monkeys)):
        ...
        monkey.items.clear()
        for item in items:
            item = monkey.operation(item) % monkey_modulus
