...
RepeatInfo = NamedTuple('RepeatInfo', [
    ('rocks', int),
    ('height', int),
])

repeat_dict: dict[int, dict[bytes, RepeatInfo]] = {}
# for a given tick, what tower tops have what info?
INTERESTING_TOP = 100
height_adjust = 0
repeton: Optional[RepeatInfo] = None

def stop_rock() -> None:
    global current_rock
    global rock_count
    global repeton
    global height_adjust
    rock_bytes = current_rock[position]
    for i in range(len(rock_bytes)):
    ...
    while chamber[-1] == 0:
        chamber.pop()
    if tick not in repeat_dict:
        repeat_dict[tick] = {}
    tower_top = bytes(chamber[-INTERESTING_TOP:])
    repeat_info = RepeatInfo(rock_count, len(chamber))
    if tower_top in repeat_dict[tick]:
        prev_repeat = repeat_dict[tick][tower_top]
        print(f"found repeton {prev_repeat} at {repeat_info}")
        repeton = RepeatInfo(
            repeat_info.rocks - prev_repeat.rocks,
            repeat_info.height - prev_repeat.height,
        )
        print(f"rocks in repeton: {repeat_info.rocks - prev_repeat.rocks}")
        print(f"height in repeton: {repeat_info.height - prev_repeat.height}")
        rocks_left = ROCK_LIMIT - rock_count
        reps_left = rocks_left//repeton.rocks
        print(f"adding {reps_left} repetons")
        rock_count += repeton.rocks * reps_left
        height_adjust += repeton.height * reps_left
        print(f"new rock_count is {rock_count}")
        repeat_dict.clear()
    else:
        repeat_dict[tick][tower_top] = repeat_info
...
while rock_count < ROCK_LIMIT:
    ...
    tick += 1
    tick %= len(jets)
print(height_adjust + len(chamber))
