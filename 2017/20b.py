...
def tick(step_num: int) -> None:
    ...
    # find collisions
    position_count: dict[Position, int] = defaultdict(lambda: 0)
    for particle in particles:
        position_count[particle.p] += 1
    collisions = {k:v for k, v in position_count.items() if v > 1}
    if collisions:
        print(step_num, collisions)
    dead: list[Particle] = []
    for particle in particles:
        if position_count[particle.p] > 1:
...
for i in range(1000):
    ...
    if len(arrangement) > 3 and arrangement[-1] == arrangement[-2] == arrangement[-3]:
        print(f"% at {i=}, {len(arrangement[-1])=}, arrangement is {arrangement[-1][:10]}")
        break
