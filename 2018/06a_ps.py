...
area_sizes: dict[Position, list[Position]] = defaultdict(list)
...
for x in range(min_x, max_x + 1):
    for y in range(min_y, max_y + 1):
        ...
        if len(closest_location) == 1:
            ...
            area_sizes[source].append(current_point)
...
# print(area_sizes)
print("/areas [")
for (source, locations) in area_sizes.items():
    print("    [")
    for location in locations:
        print("        [", " ".join([str(c) for c in location]), "]")
    print("    ]")
print("] def")
